...
The following adapts an implementation by Matthew Wang who proposes an investment strategy that leverages statistical modeling to take advantage of volatility clustering in financial time series data. He focuses on identifying different market regimes using a Hidden Markov Model (HMM) and then adapting the investing model based on the current market regime - although his implementation was confined to switching between Growth and Fama-French 3-factor models. However, the Fama-French framework's plight is well documented which Robert D. Arnott and his co-authors attribute to poor factor definitions. To cater for this, I adapt the original code and make more explicit the Value and Growth Factors as well as apply sector neutralization to reduce the distributional effects of each factor in biasing the stock selection. Additionally, Matthew Wang original framework is dependent on Leverage applied in a long-short framework. This application is long only.
